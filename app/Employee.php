...
class Employee extends Model
{
                ...
                if(count($employees) == 0) {
                    ...
                    if(!empty($employees)) {
                        ...
                        if($leave_type == 1) {
                            ...
                            $table_date[] = '<div class="badge badge-soft-info" data-toggle="tooltip" title="'.$tooltip.'" '.$ajax_pop.'>Y</div>';
                        }
                        if($leave_type == 2) {
                            $tooltip = __('Other Leave').'  '.$start_date.' &nbsp; - &nbsp; '.$end_date;
                            $table_date[] = '<div class="badge badge-soft-success" data-toggle="tooltip" title="'.$tooltip.'" '.$ajax_pop.' >Y</div>';
                        }
                    }
                }
            }
    ...
    public function getWorkSchedule($id = 0, $week = 0, $location_id = 0)
    {
                ...
                else
                {
                    if(!empty($leave)) 
                    {
                        ...
                        if($leave_type == 1)
                        {
                            ...
                            $leave_tooltip2 = '<div class="text-center text-info holiday_leave" '.$leave_sts.' title="'.$tooltip.'">'.__('Holiday Leave').'</div>';
                        }
                        if($leave_type == 2)
                        {
                            $tooltip = $employee['first_name'].' '.__('has').' '.__('other leave').' for '.$leave_date;
                            $leave_tooltip2 = '<div class="text-center text-success other_leave" '.$leave_sts.' title="'.$tooltip.'">'.__('Other Leave').'</div>';                                                                
                        }                            
                    }
                }

                $rotas_time[$key] = $leave_tooltip2;
        ...
        return '<tr class="d-nowne" data-user-id="'.$id.'">
                    <td class="text-center">'.$user_profile_img.'<div><span>'.$employee['first_name'].' '.$employee['last_name'].'</span><br>'.$time_counter.' </div></td>
                    '.$table_date[0].'
                    '.$table_date[1].'
                    '.$table_date[2].'
                    '.$table_date[3].'
